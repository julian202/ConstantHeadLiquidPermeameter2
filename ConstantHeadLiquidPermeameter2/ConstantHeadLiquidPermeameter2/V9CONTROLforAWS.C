...
/*
cofunc void get_weightAWSContinuous()
{
   int g;
   g=serCgetc();
   serBputc(g);
...
cofunc int read_scale()  // - Mix
{
	int c, loops;
   long end_time;
   // This scale uses a parity bit so we have to adjust for that
	serCopen(9600);
   serCdatabits(PARAM_7BIT);
   serCparity(PARAM_EPARITY);
   DelayMs(10);
   loops = 0;
   // The scale will either respond immediately or never; don't know why
   while (loops < 20)
   {
      loops = loops + 1;
      // Wipe out anything from last time
      serCrdFlush();
      // Define when long is too long
   	end_time = TICK_TIMER + 100;
      // Command the scale to read
      ...
      // Wait for a character in the buffer that means something
      waitfor(serCpeek() == 'S' || serCpeek() == 'Q' || serCpeek() == 'O' || TICK_TIMER > end_time);
      // If it's not a normal kind of reading, throw
      if (serCpeek() == 'Q' || serCpeek == 'O') return -2;
      // If it did not respond immediately, we need to try again
      if (TICK_TIMER > end_time) continue;
      // Get and send characters
	   c = serCgetc();
	   while(c != 10)
	   {
      	// Only send relevant characters
	      if (c >= '+' && c <= '9' && c != ',' && c != '/')
	      {
	         serBputc(c);
	      }
...
cofunc void get_weightANDcontinuous()
{
int c;
   int last;
   //long start_time;
   ...
   for (times=0; times<10000; times++)
   {
      c=serCgetc();
      /*
      //get the 11 next chars after "W" or 9 next chars after "WT:". (87 is 'W', 84 is 'T')
      if(c==87) //87 is 'W'
      {
         while(c!=13){
            c=serCgetc();
...
{//open main
   ...
   serCopen(9600);
   serCdatabits(PARAM_7BIT); //PARAM_8BIT   //PARAM_7BIT
   serCflowcontrolOff();
   serCparity(PARAM_EPARITY);  //PARAM_NOPARITY //PARAM_EPARITY
   ...
   {//open for 1
      ...
      {//open costate 1
         ...
         //for continuous reading from AWS weigh scale:
         //serCopen(9600);
	      //f=serCgetc();
	      //serBputc(f);
         //
         ...
         {// open switch 1
            ...
            //Scale control.
            case 'M':
               wfd read_write = input_character(1000,1);
               //wfd channel = input_character(1000, 1);
               switch(read_write)
               {
                  case 'R':
                     //wfd get_weight(channel-'1');
                     wfd get_weightAWS();   //get_weightAND
                     break;

                  case 'Q':
                     wfd get_weightANDcontinuous();
                     break;

                  case 'S':
                     // Try to read the scale
         	         // Remember that AND scales have settings that need to be set to work:
	                  // The baud rate must be set to 9600
	                  // You likely have to set it in "Command Mode"
	                  // These settings are manipulated on the scale
	                  wfd j = read_scale();
	                  // If it reads but does not give the right kind of reading, throw
	                  if (j == -2) serBputs("e00000.00");
	                  // If it does not read, throw
	                  else if (j != 1) serBputs("E00000.00");
	                  break;
